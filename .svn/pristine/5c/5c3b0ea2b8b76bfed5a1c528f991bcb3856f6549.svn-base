 ...
 */
@WebServlet(description = "规则结果下发", urlPatterns = { "/TeacherRulesInfoList" })
public class TeacherRulesInfoList extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger logger=Logger.getLogger(TeacherRulesInfoList.class);
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TeacherRulesInfoList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while(resultSet.next()) {
				new JieciCommon();
				JieciCommon jieciCommon = JieciCommon.teacherbuild(resultSet.getString("jieciid"), new JieciTypeUtils().GetCSSClassname(resultSet.getInt("type")), resultSet.getString("teachername"), resultSet.getString("teacherid"));
