...
public class Blowfish
{
	...
	public Blowfish(String password)
	{
		...
		this.m_bfish = new BlowfishCBC(digest.digest(), 0L);
		digest.reset();
	}
	...
	private String encStr(String sPlainText, long lNewCBCIV)
	{
		...
		{
			char cActChar = sPlainText.charAt(i);
			buf[(nPos++)] = ((byte)(cActChar >> '\b' & 0xFF));
	...
	private static class BlowfishECB
	{
		...
		public BlowfishECB(byte[] bfkey)
		{
			...
			for (int i = 0; i < 256; i++)
			{
				this.m_sbox1[i] = sbox_iit_1[i];
				this.m_sbox2[i] = sbox_iit_2[i];
				this.m_sbox3[i] = sbox_iit_3[i];
				this.m_sbox4[i] = sbox_iit_4[i];
			}
			...
			for (int i = 0; i < 18; i++)
			{
				...
				this.m_pbox[i] ^= nBuild;
			}
			long lZero = 0L;
			for (int i = 0; i < 18; i += 2)
			{
				lZero = encryptBlock(lZero);
				this.m_pbox[i] = ((int)(lZero >>> 32));
				this.m_pbox[(i + 1)] = ((int)(lZero & 0xFFFFFFFF));
			}
			for (int i = 0; i < 256; i += 2)
			{
				lZero = encryptBlock(lZero);
				this.m_sbox1[i] = ((int)(lZero >>> 32));
				this.m_sbox1[(i + 1)] = ((int)(lZero & 0xFFFFFFFF));
			}
			for (int i = 0; i < 256; i += 2)
			{
				lZero = encryptBlock(lZero);
				this.m_sbox2[i] = ((int)(lZero >>> 32));
				this.m_sbox2[(i + 1)] = ((int)(lZero & 0xFFFFFFFF));
			}
			for (int i = 0; i < 256; i += 2)
			{
				lZero = encryptBlock(lZero);
				this.m_sbox3[i] = ((int)(lZero >>> 32));
				this.m_sbox3[(i + 1)] = ((int)(lZero & 0xFFFFFFFF));
			}
			for (int i = 0; i < 256; i += 2)
			{
				lZero = encryptBlock(lZero);
				this.m_sbox4[i] = ((int)(lZero >>> 32));
				this.m_sbox4[(i + 1)] = ((int)(lZero & 0xFFFFFFFF));
			}
		}
		...
		public void cleanUp()
		{
			...
			for (int i = 0; i < 256; i++) {
				this.m_sbox1[i] = (this.m_sbox2[i] = this.m_sbox3[i] = this.m_sbox4[i] = 0);
			}
		}

		public static boolean selfTest()
		{
			byte[] testKey1 = { 28, 88, Byte.MAX_VALUE, 28, 19, -110, 79, -17 };

			int[] tv_p1 = { 810889768, 1836001626 };
			int[] tv_c1 = { 1439381364, -784403967 };
			int[] tv_t1 = new int[2];

			String sTestKey2 = "Who is John Galt?";
			byte[] testKey2 = sTestKey2.getBytes();

			int[] tv_p2 = { -19088744, 1985229328 };
		...
		protected long decryptBlock(long lCipherBlock)
		{
			...
			nHi ^= this.m_pbox[17];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[16];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[15];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[14];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[13];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[12];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[11];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[10];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[9];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[8];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[7];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[6];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[5];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[4];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[3];
			nLo ^= (this.m_sbox1[(nHi >>> 24)] + this.m_sbox2[(nHi >>> 16 & 0xFF)] ^ this.m_sbox3[(nHi >>> 8 & 0xFF)]) + this.m_sbox4[(nHi & 0xFF)] ^ this.m_pbox[2];
			nHi ^= (this.m_sbox1[(nLo >>> 24)] + this.m_sbox2[(nLo >>> 16 & 0xFF)] ^ this.m_sbox3[(nLo >>> 8 & 0xFF)]) + this.m_sbox4[(nLo & 0xFF)] ^ this.m_pbox[1];

			return Blowfish.makeLong(nHi, nLo ^ this.m_pbox[0]);
		}
		...
		public void encrypt(byte[] inbuffer, byte[] outbuffer)
		{
			...
			{
				long lTemp = Blowfish.byteArrayToLong(inbuffer, i);
				lTemp = encryptBlock(lTemp);
				Blowfish.longToByteArray(lTemp, outbuffer, i);
			}
		}
		...
		public void encrypt(byte[] buffer)
		{
			...
			for (int i = 0; i < nLen; i += 8)
			{
				long lTemp = Blowfish.byteArrayToLong(buffer, i);
				lTemp = encryptBlock(lTemp);
				Blowfish.longToByteArray(lTemp, buffer, i);
			}
		}
		...
		public void encrypt(int[] inbuffer, int[] outbuffer)
		{
			...
			{
				long lTemp = Blowfish.intArrayToLong(inbuffer, i);
				lTemp = encryptBlock(lTemp);
				Blowfish.longToIntArray(lTemp, outbuffer, i);
			}
		}
		...
		public void encrypt(int[] buffer)
		{
			...
			for (int i = 0; i < nLen; i += 2)
			{
				long lTemp = Blowfish.intArrayToLong(buffer, i);
				lTemp = encryptBlock(lTemp);
				Blowfish.longToIntArray(lTemp, buffer, i);
			}
		}
		...
		public void encrypt(long[] inbuffer, long[] outbuffer)
		{
			...
			for (int i = 0; i < nLen; i++) {
				outbuffer[i] = encryptBlock(inbuffer[i]);
			}
		}
		...
		public void encrypt(long[] buffer)
		{
			int nLen = buffer.length;
			for (int i = 0; i < nLen; i++) {
				buffer[i] = encryptBlock(buffer[i]);
			}
		}
		...
		public void decrypt(long[] inbuffer, long[] outbuffer)
		{
			int nLen = inbuffer.length;
			for (int i = 0; i < nLen; i++) {
				outbuffer[i] = decryptBlock(inbuffer[i]);
			}
		}
	...
	extends Blowfish.BlowfishECB
	{
		...
		{
			Blowfish.longToByteArray(this.m_lCBCIV, dest, 0);
		}
		...
		{
			this.m_lCBCIV = Blowfish.byteArrayToLong(newCBCIV, 0);
		}
		...
		public BlowfishCBC(byte[] bfkey, long lIitCBCIV)
		{
			super(bfkey);
		...
		public BlowfishCBC(byte[] bfkey, byte[] iitCBCIV)
		{
			super(bfkey);
		...
		private long encryptBlockCBC(long lPlainblock)
		{
			...
			return this.m_lCBCIV = lPlainblock;
		}
		...
		public void encrypt(long[] buffer)
		{
			int nLen = buffer.length;
			for (int i = 0; i < nLen; i++) {
				buffer[i] = encryptBlockCBC(buffer[i]);
			}
		}
		...
		public void decrypt(long[] inbuffer, long[] outbuffer)
		{
			int nLen = inbuffer.length;
			for (int i = 0; i < nLen; i++) {
				outbuffer[i] = decryptBlockCBC(inbuffer[i]);
			}
		}
		...
		public void decrypt(long[] buffer)
		{
			int nLen = buffer.length;
			for (int i = 0; i < nLen; i++) {
				buffer[i] = decryptBlockCBC(buffer[i]);
			}
		}
	}
	...
	{
		return buffer[nStartIndex] << 56 | (buffer[(nStartIndex + 1)] & 0xFF) << 48 | (buffer[(nStartIndex + 2)] & 0xFF) << 40 | (buffer[(nStartIndex + 3)] & 0xFF) << 32 | (buffer[(nStartIndex + 4)] & 0xFF) << 24 | (buffer[(nStartIndex + 5)] & 0xFF) << 16 | (buffer[(nStartIndex + 6)] & 0xFF) << 8 | buffer[(nStartIndex + 7)] & 0xFF;
	}

	private static void longToByteArray(long lValue, byte[] buffer, int nStartIndex)
	{
		buffer[nStartIndex] = ((byte)(int)(lValue >>> 56));
		buffer[(nStartIndex + 1)] = ((byte)(int)(lValue >>> 48 & 0xFF));
		buffer[(nStartIndex + 2)] = ((byte)(int)(lValue >>> 40 & 0xFF));
		buffer[(nStartIndex + 3)] = ((byte)(int)(lValue >>> 32 & 0xFF));
		buffer[(nStartIndex + 4)] = ((byte)(int)(lValue >>> 24 & 0xFF));
		buffer[(nStartIndex + 5)] = ((byte)(int)(lValue >>> 16 & 0xFF));
		buffer[(nStartIndex + 6)] = ((byte)(int)(lValue >>> 8 & 0xFF));
		buffer[(nStartIndex + 7)] = ((byte)(int)lValue);
	}

	private static long intArrayToLong(int[] buffer, int nStartIndex)
	{
		return buffer[nStartIndex] << 32 | buffer[(nStartIndex + 1)] & 0xFFFFFFFF;
	}
	...
	private static String bytesToBinHex(byte[] data, int nStartPos, int nNumOfBytes)
	{
		...
		{
			sbuf.setCharAt(nPos++, HEXTAB[(data[(i + nStartPos)] >> 4 & 0xF)]);
			sbuf.setCharAt(nPos++, HEXTAB[(data[(i + nStartPos)] & 0xF)]);
		}
		...
		for (int i = 0; i < nNumOfBytes; i++)
		{
			...
			for (int nJ = 0; nJ < 2; nJ++)
			{
				...
				if ((cActChar >= 'a') && (cActChar <= 'f')) {
					bActByte = (byte)(bActByte | (byte)(cActChar - 'a') + 10);
				} else if ((cActChar >= '0') && (cActChar <= '9')) {
					bActByte = (byte)(bActByte | (byte)(cActChar - '0'));
				} else {
					blConvertOK = false;
				}
			}
